...
from gpt_transformer.decoding import Decoding
...
def main(type_):
    ...
    if type_ == 1:
        gen_poem_ut = Decoding("models/gpt_transformer/checkpoints/ki-009.pt")
        print("Loaded normal model")
    else:
        gen_poem_ut = Decoding("models/gpt_transformer/checkpoints_nws/ki-009.pt")
        print("Loaded No Xtra White Space model")
    ...
    if type_ == 1:
        with open("models/gpt_transformer/scores/score_final.json", "w") as f:
            json.dump(results, f)
    else:
        with open("models/gpt_transformer/scores/score_nws_final.json", "w") as f:
            json.dump(results, f)
