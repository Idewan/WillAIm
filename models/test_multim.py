...
from m_adv.code.src.utils import GenPoemUtilsMultiM
...
#Run with conda multi-m environment - python2.7 and mxnet
...
def main():
    ...
    clip_eval = CLIPEval()
    gen_poem_ut = GenPoemUtilsMultiM()
    ...
    object_model = gen_poem_ut.get_mod()
    object_model.load_params('m_adv/code/model/object.params')
    
    scene_model = gen_poem_ut.get_mod()
    scene_model.load_params('m_adv/code/model/scene.params')

    sentiment_model = gen_poem_ut.get_mod(sym = symbol_sentiment.get_sym(), img_len = 227)
    ...
    for i in tqdm(range(len(test_data))):
        ...
        img_feature = extract_feature(test_data[i]["img_path"])
        pred_poem = '\n' + gen_poem_ut.generate_poem(img_feature)[0].replace('\n', '\n'
